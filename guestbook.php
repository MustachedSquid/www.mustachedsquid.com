    ...
    if(isset($_POST['submitInput']) && isset($_POST['nameInput']) && isset($_POST['messageInput'])){
        ...
        if(strlen(trim($_POST["nameInput"])) > 0 && strlen(trim($_POST['messageInput'])) > 0){
            if(strlen($_POST["nameInput"]) < 32 && strlen($_POST['messageInput']) < 254){
                ...
                if(preg_match("/^[\r\n|\n|\ráàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ!?.,a-zA-Z0-9_ ]+?$/", $_POST["nameInput"])){

                    if(preg_match("/^[\r\n|\n|\ráàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ!?.,a-zA-Z0-9_ ]+?$/", $_POST["messageInput"])){
                        ...
                        $html = "<div class='sections'>Error: Message has inválid characters</div>";
                    }
                }else{
                    $html = "<div class='sections'>Error: Name has inválid characters</div>";
                }
            }else{
